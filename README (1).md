...
- Two drop zones: Quicken exports (left) and Detail files (right)
- Quicken: `.csv`, `.qif`, `.qfx`, `.ofx` — all supported
- Detail files: Amazon order history, Apple Card monthly CSV, or any CSV with Date + Description + Amount
- Auto-detects detail file format — Amazon, Apple Card, or generic enrichment
- **Purchaser inference:** if a filename contains a family member's first name (e.g. `amazon_chris.csv`), every item in that file is attributed to that person
- Automatic deduplication — always safe to re-import; purchaser is part of the dedup key
- Per-file progress bar, rich file cards with purchaser/source badges, full clear controls
...
**Detail Lens — Line-Item Intelligence** *(formerly Amazon Watchlist)*
...
### First import — two files, one drop

```
The Pour page has two zones:

Left zone  → Quicken export (all accounts, full history)
Right zone → Detail files (Amazon, Apple Card, etc.) — optional but powerful
```
